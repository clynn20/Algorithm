...
void filledgeinfo(Graph* graph,Vertex* v_arr){
...
// mergearr function from hw1
void mergearr(Edge* array,int left,int mid,int right){
	int n1 = mid-left+1;
	int n2 = right-mid;
	Edge leftarr[n1];
	Edge rightarr[n2];
...
// funcion to print the vertice array 
void printv_arr(Vertex* v_arr,int V){	
...
void printe_arr(Graph* graph,int E){
...
int main(){
	...
	if(infile.is_open()){
			...
			for(int i=0;i<cases;i++){
				...
				//printv_arr(v_arr,V); for check only
				filledgeinfo(graph,v_arr);
				//printe_arr(graph,E); for check only
